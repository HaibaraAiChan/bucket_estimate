...
def estimate_mem(data_dict, in_feat, hidden_size, redundant_ratio):	
	...
	for idx in range(len(redundant_ratio)):
		modified_estimated_mem_list.append(estimated_mem_list[idx]*redundant_ratio[idx]) 
		# redundant_ratio[i] is a variable depends on graph characteristic
		print(' MM estimated memory/GB item '+str(idx)+': '+str(estimated_mem_list[idx]) + " * " +str(redundant_ratio[idx]) ) 
	...
	for run in range(args.num_runs):
		...
		for epoch in range(args.num_epochs):
			...
			if args.num_batch > 1:
				...
				data_dict = []
				print('redundancy ratio #input/#seeds/degree/cluster_coeff')
				redundant_ratio = []
				for step, (input_nodes, seeds, blocks) in enumerate(b_block_dataloader):
					print(len(input_nodes)/len(seeds)/100/0.411)
					redundant_ratio.append(len(input_nodes)/len(seeds)/100/0.411)
				...
				print('1-40 degree')
				print(modified_res[0])
				print('41-55 degree')
				print(modified_res[1])
				print('55-66 degree')
				print(modified_res[2])
				print('66-75 degree')
				print(modified_res[3])
				print('75-83 degree')
				print(modified_res[4])
				print('83-91 degree')
				print(modified_res[5])
				print('91-99 degree')
				print(modified_res[6])
				print('100 degree split 34 parts')
				print(modified_res[7:40])
...
def main():
	...
	argparser.add_argument('--selection-method', type=str, default='100__bucketing')
	argparser.add_argument('--num-batch', type=int, default=42)

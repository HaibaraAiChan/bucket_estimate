...
def get_shared_numbers(lst):
    # Use itertools.combinations to get all pairs of sublists
    # print(lst)
	...
	for i in range(len(lst)):
		...
		# Use set intersection to find the shared numbers
			tensor1 = lst[i]
			tensor2 = lst[j]
			print('tensor ', i)
			print('tensor ', j)
			equal_elements = torch.eq(tensor1[:, None], tensor2)
			# Get the indices of equal elements
			equal_indices = torch.nonzero(equal_elements)
			# Extract the shared items using the indices
			shared_items = torch.unique(tensor1[equal_indices[:, 0]])

			length = len(shared_items)
			# print(f"The shared numbers of a[{tensor1}] and a[{tensor2}] is {length}")
			print(f"The shared numbers of tensor{i} and tensor{j} is {length}")
			length_list.append(length)
	return length_list


def similarity(sublists):
    length_list = get_shared_numbers(sublists)
    print(length_list)
    return length_list

# def greedy_set_cover(sublists):
#     universe = set().union(*sublists)  # Create the universe of all elements
    # sublists = [set(sublist) for sublist in sublists]  # Convert each sublist to a set
    # selected_sublists = []
    # pdb.set_trace()
    # # While there are elements remaining in the universe
    # while universe:
    #     # Find the sublist that covers the most uncovered elements
    #     best_sublist = max(sublists, key=lambda sublist: len(sublist & universe))
        
    #     # Remove the elements of the best sublist from the universe
    #     universe -= best_sublist

    #     # Add the best sublist to the selected sublists
    #     selected_sublists.append(best_sublist)

    # return selected_sublists

# # Example usage:
# a = [[1, 2, 3], [2, 3, 4], [3, 5, 7]]
# result = greedy_set_cover(a)
# print("Selected sublists (sets):", result)
		...
		for epoch in range(args.num_epochs):
			...
			if args.num_batch > 1:
				...
				for step, (input_nodes, seeds, blocks) in enumerate(b_block_dataloader):
					...
					print('the length of current degree ', len(input_nodes))
				result = similarity(input_nodes_list)
				print("shared neighor numbers:", result)
				# 	layer = 0
				# 	for b in blocks:
				# 		# print('layer ', layer)
				...
				# 	print()
				# for step, (input_nodes, seeds, blocks) in enumerate(b_block_dataloader):
					# print('degree\t '+ str(step+1) + ' \t redundancy \t'+str(len(input_nodes)/len(seeds)/(step+1)))
